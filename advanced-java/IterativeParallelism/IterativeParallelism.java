...
public class IterativeParallelism implements ListIP {
    private <T> List<List<? extends T>> getThreadPart(int threads, List<? extends T> values) {
    ...
    private <T, U> U doParallel(int threads,
                                List<? extends T> values,
                                Function<Stream<? extends T>, ? extends U> calcFunction,
                                Function<Stream<? extends U>, ? extends U> resultCollector) throws InterruptedException {
        threads = Math.min(threads, values.size());

        Thread[] threadList = new Thread[threads];
        List<U> result = new ArrayList<>(Collections.nCopies(threads, null));
        List<List<? extends T>> parts = getThreadPart(threads, values);

        for (int i = 0; i < threads; ++i) {
            int index = i;
            threadList[index] = new Thread(() -> result.set(index, calcFunction.apply(parts.get(index).stream())));
