...
double RAD_DEG(const double current, const bool rad_on)
{
    if (rad_on) {
        return current;
    }
...
double DEG_RAD(const double current, const bool rad_on)
{
    if (rad_on) {
        return current;
    }
...
bool nullary(Op op, bool & rad_on)
{
    if (op == Op::RAD) {
        rad_on = true;
    }
...
double unary(const double current, const Op op, const bool & rad_on)
{
    ...
    case Op::SIN:
        return sin(RAD_DEG(current, rad_on));
    case Op::COS:
        return cos(RAD_DEG(current, rad_on));
    case Op::TAN:
        return tan(RAD_DEG(current, rad_on));
    case Op::CTN:
        if (current == 0) {
            return INFINITY;
        }
        return 1 / tan(RAD_DEG(current, rad_on));
    case Op::ASIN:
        if (-1 <= current && current <= 1) {
            return DEG_RAD(asin(current), rad_on);
        }
        else {
    ...
    case Op::ACOS:
        if (-1 <= current && current <= 1) {
            return DEG_RAD(acos(current), rad_on);
        }
        else {
    ...
    case Op::ATAN:
        return DEG_RAD(atan(current), rad_on);
    case Op::ACTN:
        return DEG_RAD(M_PI_2 - atan(current), rad_on);
    default:
        return current;
    }
}
