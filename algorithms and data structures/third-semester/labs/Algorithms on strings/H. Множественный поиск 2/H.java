import java.io.*;
import java.util.*;
 
public class H {
    public static class Node {
        int[] next = new int[26];
        int[] go = new int[26];
        int link = -1;
        int parent = -1;
        int parentChar = 0;
        ArrayList<Integer> terminals;
 
        public void fill(int[] a, int[] b) {
            for (int i = 0; i < a.length; ++i) {
                a[i] = -1;
                b[i] = -1;
            }
        }
 
        public Node(int link, int parent, int parentChar, ArrayList<Integer> terminals) {
            fill(next, go);
            this.link = link;
            this.parent = parent;
            this.parentChar = parentChar;
            this.terminals = terminals;
        }
 
        public Node(ArrayList<Integer> terminals) {
            this.terminals = terminals;
            fill(next, go);
        }
    }
 
    public static ArrayList<Node> tree = new ArrayList<>(List.of(new Node(new ArrayList<>())));
    public static int cnt = 0;
    public static int tSize = 1;
 
    public static void addString(String s) {
        int v = 0;
        for (int i = 0; i < s.length(); ++i) {
            int c = (s.charAt(i) - 'a');
            if (tree.get(v).next[c] == -1) {
                Node nd = new Node(-1, v, c, new ArrayList<>());
                tree.add(nd);
                tree.get(v).next[c] = tSize++;
            }
            v = tree.get(v).next[c];
        }
        tree.get(v).terminals.add(cnt++);
    }
 
    public static InputReader input = new InputReader(System.in);
 
    public static int getLink(int v) {
        if (tree.get(v).link == -1) {
            if (v == 0 || tree.get(v).parent == 0) {
                tree.get(v).link = 0;
            } else {
                tree.get(v).link = go(getLink(tree.get(v).parent), tree.get(v).parentChar);
            }
        }
        return tree.get(v).link;
    }
 
    public static int go(int v, int c) {
        if (tree.get(v).go[c] == -1) {
            if (tree.get(v).next[c] != -1) {
                tree.get(v).go[c] = tree.get(v).next[c];
            } else {
                tree.get(v).go[c] = v == 0 ? 0 : go(getLink(v), c);
            }
        }
        return tree.get(v).go[c];
    }
 
    public static void solve() {
        int n = input.nextInt();
        for (int i = 0; i < n; ++i) {
            String s = input.next();
            addString(s);
        }
        String t = input.next();
        int[] result = new int[n];
        int[] used = new int[tree.size()];
        ArrayList<Integer> order = new ArrayList<>(List.of(0));
        int i = 0;
        while (i != order.size()) {
            int u = order.get(i++);
            for (int v : tree.get(u).next) {
                if (v != -1) {
                    order.add(v);
                }
            }
        }
        Collections.reverse(order);
        int cur = 0;
        for (int j = 0; j < t.length(); ++j) {
            cur = go(cur, (t.charAt(j) - 'a'));
            used[cur] += 1;
        }
        for (int v : order) {
            int link = getLink(v);
            used[link] += used[v];
        }
        for (int v : order) {
            for (int terminal : tree.get(v).terminals) {
                result[terminal] = used[v];
            }
        }
        OutputStream outputStream = System.out;
        PrintWriter out = new PrintWriter(outputStream);
        for (int el : result) {
            out.println(el);
        }
        out.close();
    }
 
    public static void main(String[] args) {
        solve();
    }
 
    public static class InputReader {
        public BufferedReader reader;
        public StringTokenizer tokenizer;
 
        public InputReader(InputStream stream) {
            reader = new BufferedReader(new InputStreamReader(stream), 32768);
            tokenizer = null;
        }
 
        public String next() {
            while (tokenizer == null || !tokenizer.hasMoreTokens()) {
                try {
                    tokenizer = new StringTokenizer(reader.readLine());
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
            }
            return tokenizer.nextToken();
        }
 
        public int nextInt() {
            return Integer.parseInt(next());
        }
    }
}