...
public class B {
    ...
    public static void dfs1(int v) {
        ...
        for (int e : graph.get(v)) {
            if (edges.get(e).getFlow() < edges.get(e).getCapacity() && !reached[edges.get(e).getV()]) {
                dfs1(edges.get(e).getV());
            }
        }
    }
    ...
    public static void generateOutput() {
        ...
        int minCutFlow = maxFlow();
        dfs1(s);
        ArrayList<Integer> cut = new ArrayList<>();
        for (int i = 0; i < 2 * m; i += 2) {
            if ((reached[edges.get(i).getU()] && !reached[edges.get(i).getV()]) || (!reached[edges.get(i).getU()] && reached[edges.get(i).getV()])) {
                cut.add(i / 2);
            }
        }
        printWriter.println(cut.size() + " " + minCutFlow);
        for (int e : cut) {
