...
public class ArticleRepositoryImpl implements ArticleRepository {
    ...
    public void save(Article article) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
                    "INSERT INTO `Article` (`userId`, `title`, `text`, `creationTime`, `hidden`) VALUES ( ?, ?, ?, NOW(), false)",
    ...
    public void updateHidden(long id, boolean hidden) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            ...
            throw new RepositoryException("Can't update hidden state of article.", e);
        }
    }
    ...
    public List<Article> findAll() {
        return findAllBySql("SELECT * FROM Article ORDER BY creationTime DESC");
    }

    @Override
    public List<Article> findAllNotHidden() {
        return findAllBySql("SELECT * FROM Article WHERE hidden=0 ORDER BY creationTime DESC");
    }

    @Override
    public List<Article> findAllByUserId(long userId) {
        return findAllBySql("SELECT * FROM Article WHERE userId=?", userId);
    }

    private List<Article> findAllBySql(String sql, Object... args) {
        List<Article> articles = new ArrayList<>();
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(sql)) {
                int cnt = 0;
                for (Object arg : args) {
                    statement.setObject(++cnt, arg);
                }
