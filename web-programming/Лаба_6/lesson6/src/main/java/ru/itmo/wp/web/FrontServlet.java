...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        for (Method action : new Method[]{before, method, after}) {
            try {
                action.setAccessible(true);
                action.invoke(page, request, view);
            } catch (IllegalAccessException e) {
                throw new ServletException("Unable to run action [pageClass=" + pageClass + ", method=" + action + "].", e);
