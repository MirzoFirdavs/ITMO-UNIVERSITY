...
class HttpServer {
    ...
    public Response generate(Response response, Request request, File file) throws IOException {
        byte[] body = readFile(file);
        response.setStatusCode(200);
        if (request.getHeader("If-None-Match") != null && request.getHeader("If-None-Match").equals(Long.toString(Arrays.hashCode(body)))) {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        if (file.isFile()) {
            response = generate(response, request, file);
