    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            method = Arrays.stream(clazz.getDeclaredMethods())
                    .filter(m -> m.getName().equals(route.getAction())
                            ...
                            .allMatch(x -> Map.class.equals(x) || HttpServletRequest.class.equals(x)))
                    .findAny()
                    .orElse(null);
        }
        /**/
        ...
        try {
            method.invoke(page, Arrays.stream(method.getParameterTypes())
                    .map(clazz -> Map.class.equals(clazz) ? view : request)
                    .toArray());
        ...
        if (lang == null || parameter != null && parameter.length() == 2) {
            lang = Optional.ofNullable(parameter).orElse("en");
            session.setAttribute("lang", lang);
        }
        /**/
        Template template = newTemplate(pageClass.getSimpleName() + ".ftlh", Locale.forLanguageTag(lang));
