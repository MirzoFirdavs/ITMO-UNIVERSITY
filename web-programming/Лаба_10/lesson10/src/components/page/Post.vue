<template>
  <article>
    <a href="#" style="text-decoration: none">
      <div class="title" @click.prevent="showPost"> {{ post.title }}</div>
    </a>
    <div class="information">By {{ users[post.userId].name }}</div>
    <div class="body">{{ post.text }}</div>
    <ul class="attachment">
      <li>Announcement of <a href="#">Codeforces Round #510 (Div. 1)</a></li>
      <li>Announcement of <a href="#">Codeforces Round #510 (Div. 2)</a></li>
    </ul>
    <div class="footer">
      <div class="left">
        <img src="../../assets/img/voteup.png" title="Vote Up" alt="Vote Up"/>
        <span class="positive-score">+173</span>
        <img src="../../assets/img/votedown.png" title="Vote Down" alt="Vote Down"/>
      </div>
      <div class="right">
        <img src="../../assets/img/date_16x16.png" title="Publish Time" alt="Publish Time"/>
        <img src="../../assets/img/comments_16x16.png" title="Comments" alt="Comments"/>
        <a href="#" style="padding: 0.2rem"> {{ commentsSize(post.id) }}</a>
      </div>
    </div>
    <Comments v-if="showComments && commentsSize(post.id) > 0" :users="users" :postComments="postComments(post.id)"/>
  </article>
</template>

<script>
import Comments from "./Comments";
export default {
  name: "Post",
  components: {
    Comments
  },
  props: ["post", "users", "comments", "showComments"],
  methods: {
    postComments: function (id) {
      return Object.values(this.comments).filter(c => c.postId === id);
    },
    commentsSize: function (id) {
      return Object.values(this.comments).filter(p => p.postId === id).length;
    },
    showPost: function () {
      this.$root.$emit("onShowPost", this.post);
    }
  }
}
</script>

<style scoped>
</style>